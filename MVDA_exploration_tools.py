...
#        return self.n_components 
    ...
    def SSX(self):
        if self.is_fitted:
            if not self.SSX_.size and (self.X_model.size and self.Y_model.size):
                self.SSX_, self.S2X_ = self.get_model_SSX_S2X(self.X_model, self.Y_model, 
                                                              self.n_components, 
    ...
    def S2X(self):
        if self.is_fitted:
            if not self.S2X_.size and (self.X_model.size and self.Y_model.size):
                self.SSX_, self.S2X_ = self.get_model_SSX_S2X(self.X_model, self.Y_model, 
                                                              self.n_components, 
                                                              is_UVscaled=self.scale, 
                                                              is_MeanCentered=self.is_MeanCentered)
...
#        print('ObsResiduals.shape ', ObsResiduals.shape)
    ...
    def DModXpred(self, X_pred_set):
        if self.is_fitted:
            Obs_resid = self.Obs_residual_SD(self.Epred(X_pred_set))
            return Obs_resid
